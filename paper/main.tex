...
re-expanded; a summary cannot), security (provenance and identity travel with the reference), and
systematization at the protocol layer. \textbf{(3)} A single-sample accuracy regression we initially observed on Haiku
\textbf{did not reproduce} at $n{=}5$ and is retracted --- a concrete illustration of why $n{=}1$
agent measurements are unsafe. \textbf{(4)} On the compute side, we implement and measure the third
materialization, a same-model KV-cache transfer, on seven open models: reusing a transferred prefix
cache is a real, near-lossless prefill speedup (35/36 cells byte-identical; up to $\sim$180$\times$ at
long context) but the cache is thousands of times larger than the text, so it pays off only when agents
are co-located or use grouped-query attention --- grounding TOAP's gating policy in measurement rather
than assumption. We also separate wire bytes from model tokens and show symbolic ``opcodes'' save 50\%
of bytes but only 0--17\% of tokens. We describe the protocol architecture (a two-plane wire format; a
...
engineered baseline, and whether small models pay an accuracy price for it. Our central result is a
little humbling: a good summarizing orchestrator saves more tokens than reference-minimization at the
same accuracy. TOAP therefore has to justify itself on losslessness, security, and systematization
rather than on raw token savings, and most of this paper is about establishing where each of those
actually matters.
...
\begin{enumerate}\itemsep1pt
  ...
  \item An implemented, seven-model measurement of same-model KV-cache reuse, showing where the
        compute win justifies the cache-transfer cost (\S\ref{sec:kv-results}).
...
secondary check. \textbf{KV-bridge scope:} the speedups are microbenchmarks of one prefill step on a single GPU (T4),
against full recompute --- not against provider prefix-caching, which is the production alternative and
which we do not claim to beat; ``transfer'' is local serialize/deserialize, not a real RDMA hop; and we
restrict to same-model prefix reuse with no cross-position splicing. The float16 token divergence in
one of 36 cells is a reminder that ``lossless'' here means token-identical in practice, not bit-exact
in theory. \textbf{Implementation scope:} the V2 binary control plane, capability lattice,
cache-coordinate layout, materialization-with-fallback, and the KV-bridge sidecar are implemented and
tested; what remains future work is a durable/distributed context store, message signing /
cross-reconnect replay nonces, and cross-vendor (non-Claude) replication of the token study.
