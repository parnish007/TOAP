...
\draw[msg] ($(B.south)+(0,\y-0.7)$) -- node[above,font=\scriptsize]{\ttfamily RES|7|OK(CTX:42)} ($(O.south)+(0,\y-0.7)$);
\node[note, right] at ($(B.south)+(0,\y-1.15)$) {document now lives once, as CTX:42};
% 2. request
\draw[msg] ($(O.south)+(0,\y-1.9)$) -- node[above,font=\scriptsize]{\ttfamily REQ|8|worker|SUM(CTX:42)?max\_words=60} ($(B.south)+(0,\y-1.9)$);
% broker self-checks
...
\draw[msg] ($(B.south)+(0,\y-4.3)$) -- node[above,font=\scriptsize]{\ttfamily REQ|8|SUM(CTX:42)} ($(W.south)+(0,\y-4.3)$);
\node[note] at ($(W.south)+(0,\y-4.75)$) {worker GETs CTX:42, summarizes};
\draw[msg] ($(W.south)+(0,\y-5.4)$) -- node[above,font=\scriptsize]{\ttfamily RES|8|OK(CTX:87)} ($(B.south)+(0,\y-5.4)$);
% 3. reply correlated by msg_id
\draw[msg] ($(B.south)+(0,\y-6.1)$) -- node[above,font=\scriptsize]{\ttfamily RES|8|OK(CTX:87)} ($(O.south)+(0,\y-6.1)$);
\node[note, right] at ($(B.south)+(0,\y-6.5)$) {broker matches msg\_id 8 back to its requester};
\node[note] at ($(O.south)+(0,\y-7.0)$) {orchestrator GETs CTX:87};
\end{tikzpicture}
