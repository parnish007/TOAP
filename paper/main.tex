...
(absolute positions preserved; no cross-position splicing) across an open-model matrix spanning
multi-head attention (GPT-2 family), rotary/NeoX (Pythia), and grouped-query attention
(Qwen2.5, Mistral-7B in 4-bit) on an NVIDIA T4. Timing is warmed-up, CUDA-event-timed, and isolates
\emph{prefill} (decode excluded). We report two speedups per cell: \emph{resident}
($=$ recompute prefill $/$ query-prefill on an already-resident KV --- the co-located / shared-broker
case, the pure compute win) and \emph{cross-node} (additionally charging KV deserialize/transfer).
Correctness is exact greedy-token match vs.\ recompute.

\begin{figure}[h]\centering
\includegraphics[width=0.82\textwidth]{figures/fig_kv_bridge.png}
\caption{KV-bridge prefill reuse across the open-model matrix (T4). \emph{Left:} resident prefill
speedup vs.\ shared-prefix length ($>1$ = faster than recompute). \emph{Right:} KV-cache size relative
to the text it replaces. \textbf{Figure shows a placeholder; it is regenerated from the committed
\texttt{kv\_bench\_all.json} via \texttt{paper/figures/gen\_kv\_figure.py} once the benchmark run is in.}}
\label{fig:kv}
\end{figure}

\noindent The hypothesis the figure tests (to be confirmed by the committed run): the resident speedup
exceeds $1\times$ above a crossover prefix length and grows with prefix length and model size, while
outputs stay token-lossless; because the KV cache is orders of magnitude larger than the text (and
larger for MHA than GQA), the cross-node speedup crosses $1\times$ only at long prefixes. This is
exactly why TOAP gates \textsc{KV\_Bridge} behind same-model and (effective) co-location, falling back
to \textsc{Ctx\_Ref} otherwise --- a policy choice the measurement grounds rather than assumes.
...
secondary check. \textbf{Implementation scope:} the V2 binary control plane, capability lattice,
cache-coordinate layout, materialization-with-fallback, and the same-model KV-bridge sidecar are
implemented and tested. What remains future work is a durable/distributed context store, message
signing / cross-reconnect replay nonces, and cross-vendor (non-Claude) replication of the token study.
