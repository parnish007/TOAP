...
Figure~\ref{fig:broker} draws both mechanisms.
...
\begin{figure}[H]\centering
\begin{tikzpicture}[font=\footnotesize,
  ag/.style={draw, rounded corners, fill=tgray, minimum width=1.6cm, minimum height=0.6cm},
  brk/.style={draw, rounded corners, fill=tblue!12, minimum width=2.4cm, minimum height=1.0cm, align=center},
  arr/.style={-{Stealth[length=1.8mm]}, thick},
  sl/.style={font=\scriptsize}, note/.style={font=\scriptsize\itshape, gray!55!black}]

% ===== (a) correlation by msg_id : requests on the top lane, responses on the bottom lane =====
\node[sl, anchor=west] at (-0.2,1.45) {\textbf{(a) correlation by \texttt{msg\_id}}};
\node[ag]               (a)  at (0,0.7)    {agent A};
\node[ag]               (c)  at (0,-0.7)   {agent C};
\node[brk]              (b1) at (3.6,0)    {broker\\[0.3mm]\scriptsize pending \{8$\to$A,\,9$\to$C\}};
\node[ag]               (w)  at (7.6,0)    {worker};
% requests in (top), response back to A (bottom) -- arcs keep the two directions apart
\draw[arr] (a.east)        to[bend left=10]  node[sl,above]{\ttfamily REQ|8} (b1.north west);
\draw[arr] (c.east)        to[bend right=10] node[sl,below]{\ttfamily REQ|9} (b1.south west);
\draw[arr] (b1.north east) to[bend left=10]  node[sl,above]{\ttfamily REQ|8} (w.north west);
\draw[arr] (w.south west)  to[bend left=10]  node[sl,below]{\ttfamily RES|8} (b1.south east);
\draw[arr] (b1.west)       to[bend right=22] node[sl,below]{\ttfamily RES|8} (a.south east);
\node[note] at (3.6,-1.45) {reply id 8 $\to$ A by the map, never by a name the worker knows};

% ===== (b) subscription fan-out =====
\node[sl, anchor=west] at (-0.2,-2.55) {\textbf{(b) subscription fan-out}};
\node[ag]                  (e)  at (0,-3.7)    {editor};
\node[brk, minimum height=0.7cm] (b2) at (3.6,-3.7) {broker};
\node[ag]                  (s1) at (7.6,-2.9)   {sub 1};
\node[ag]                  (s2) at (7.6,-3.7)   {sub 2};
\node[ag]                  (s3) at (7.6,-4.5)   {sub 3};
\draw[arr] (e.east)  -- node[sl,above]{\ttfamily DLT|PATCH} (b2.west);
\draw[arr] (b2.east) -- node[sl,above,pos=0.6]{\ttfamily EVT} (s1.west);
\draw[arr] (b2.east) -- node[sl,above]{\ttfamily EVT} (s2.west);
\draw[arr] (b2.east) -- node[sl,below,pos=0.6]{\ttfamily EVT} (s3.west);
\node[note] at (3.6,-4.95) {one PATCH, fanned out as EVT to every subscriber};
\end{tikzpicture}
\caption{Two broker mechanisms, drawn separately so each reads cleanly. \textbf{(a)} Correlation by
\texttt{msg\_id}: the broker keeps a pending map (id $\to$ requester), so concurrent requests from A
(id~8) and C (id~9) are untangled; the worker replies \emph{to the id}, and the broker routes
\texttt{RES|8} back to A by the map --- never by a name the worker had to know. \textbf{(b)} Subscription
fan-out: a single \texttt{PATCH} (a \texttt{DLT}) to a context is pushed as an \texttt{EVT} to every
