...
tokenizer-aligned \emph{semantic plane} optimized for tokens. Because each plane is encoded for its
single reader, neither is dominated --- which is precisely why ``bytes vs.\ tokens'' (\S\ref{sec:bytes})
stops being a confusion: they are different planes with different objectives.
...
them as one policy is the architecture's most novel point. Figure~\ref{fig:flow} shows the token-flow
contrast the experiment measures: naive prompts grow with pipeline depth while references stay flat,
...
% === AWAITING COLAB DATA: fill Table~\ref{tab:kv} + Figure from benchmark/kv_bridge/kv_bench_all.json ===
% \begin{table}[h]\centering\footnotesize
% \begin{tabular}{llrrrrc}
% \toprule
% Model & Attn & Prefix & KV/text & Speedup (resident) & Speedup (x-node) & Lossless \\
% \midrule
% ... rows from kv_bench_all.json ...
% \bottomrule
% \end{tabular}
% \caption{KV-bridge prefill reuse (T4). Resident = co-located compute win; x-node includes transfer.}
% \label{tab:kv}
% \end{table}

\noindent\emph{Result placeholder (to be filled from the committed run records).} We expect the
resident speedup to exceed $1\times$ above a crossover prefix length and to grow with prefix length and
model size, with outputs token-lossless; the KV cache is orders of magnitude larger than the text it
replaces (and larger for MHA than GQA), so the cross-node speedup crosses $1\times$ only at long
prefixes. This is exactly why TOAP gates \textsc{KV\_Bridge} behind same-model and (effective)
co-location, falling back to \textsc{Ctx\_Ref} otherwise --- a policy choice the measurement is meant to
